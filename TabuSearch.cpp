...
bool isTabu(int colorToQuery);
void initialize();
//void freeMemoryTabu();
void allocateMemoryTabu();
 ...
 * (added setting to NULL as well)                                     *
 ***********************************************************************/
void freeMemoryTabu(){
	free(actualColoring);			actualColoring = NULL;
	free(scoreColorVector);			scoreColorVector = NULL;
	free(uncoloredVertexSet);		uncoloredVertexSet = NULL;
	free(tempUncoloredVertexSet);	tempUncoloredVertexSet = NULL;
	free(tempSet);					tempSet = NULL;
	free(bestColoring);				bestColoring = NULL;
	free(tabuListTable);			tabuListTable = NULL;			//pss!
}
...
void selectColor(){
	...
	for(i=1; i<=KColorNumber; i++){
		if( (scoreColorVector[i]<minScore) &&
			( (isTabu(i)==false) ||
...
int tabuSearch(int* coloring){
	...
	//deallocation-PSS, better to do it globally!
	//freeMemoryTabu();
